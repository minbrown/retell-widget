        ...
        (async () => {
          ...
          try {
            const mapRes = await fetch("https://api.firecrawl.dev/v1/map", {
              ...
              body: JSON.stringify({ url: website, search: "services pricing about contact team", limit: 5 }),
            });

            if (mapRes.ok) {
              const mapData = await mapRes.json();
              (mapData.links || []).forEach(link => {
                // Ensure link is string and starts with http (full URL)
                if (typeof link === 'string' && link.startsWith('http') && !targetUrls.includes(link) && targetUrls.length < 4) {
          ...
          console.log(`   🔎 Scraping ${targetUrls.length} pages: ${targetUrls.join(", ")}`);
                ...
                body: JSON.stringify({
                  ...
                  jsonOptions: {
                    schema: {
                      type: "object",
                      properties: {
                        about_business: { type: "string", description: "General description of the business" },
                        services_offered: { type: "string", description: "List of services or treatments" },
                        business_hours: { type: "string", description: "Operational hours" },
                        location_details: { type: "string", description: "Address or area served" }
                      }
                    }
                  }
                }),
          ...
          results.forEach((r, index) => {
            const d = r?.data?.json;
            console.log(`   📄 Scrape result from ${targetUrls[index]}:`, d ? "Success" : "Empty/Failed");
            if (d) {
              if (d.about_business) mergedContext.push(`About: ${d.about_business}`);
              if (d.services_offered) mergedContext.push(`Services: ${d.services_offered}`);
              if (d.business_hours) mergedContext.push(`Hours: ${d.business_hours}`);
              if (d.location_details) mergedContext.push(`Location: ${d.location_details}`);
            }
          });

          scrapedKnowledge = mergedContext.length > 0
            ? mergedContext.join("\n\n")
            : "No specific details found on website. Proceed with general professional assistance.";
        })(),
        timeoutPromise
      ]);
      console.log(`   ✅ Extraction complete. Context length: ${scrapedKnowledge.length} chars`);
    } catch (err) {
...
// ── New: POST /check-availability ───────────────────────────────────────────
// This endpoint is called by Retell to find open spots on the GHL calendar.
app.post("/check-availability", async (req, res) => {
  ...
  try {
    ...
    // We'll look for slots starting from "now" up to 3 days in the future
    const now = new Date();
    const startDate = now.getTime();
    const endDate = new Date(now.getTime() + 3 * 24 * 60 * 60 * 1000).getTime(); // +3 days

    const slotsUrl = `https://services.leadconnectorhq.com/calendars/free-slots?calendarId=${process.env.GHL_CALENDAR_ID}&startDate=${startDate}&endDate=${endDate}`;

    console.log(`   📡 Fetching slots from: ${new Date(startDate).toLocaleDateString()} to ${new Date(endDate).toLocaleDateString()}`);

    const slotsRes = await fetchWithRetry(slotsUrl, { headers: GHL_HEADERS });
    const slotsData = slotsRes.ok ? await slotsRes.json() : null;

    if (!slotsRes.ok || !slotsData) {
      console.error("   ❌ GHL Slots Error:", await slotsRes.text());
      return res.status(500).json({ error: "Could not retrieve availability." });
    }

    // GHL returns an object with days as keys. We need to flatten and simplify this for the LLM.
    // We only want to send back the next few available times so we don't overwhelm the AI.
    let simplifiedSlots = [];
    const days = Object.keys(slotsData);

    for (const day of days) {
      const daySlots = slotsData[day]?.slots || [];
      daySlots.forEach(slot => {
        if (simplifiedSlots.length < 10) { // Limit to 10 slots
          simplifiedSlots.push(slot); // slot is an ISO string
        }
      });
    }

    console.log(`   ✅ Found ${simplifiedSlots.length} available slots.`);
    return res.json({
      available_slots: simplifiedSlots,
      message: simplifiedSlots.length > 0 ? "Here are some available times." : "No slots found in the next 3 days."
    });

  } catch (err) {
    console.error("   ❌ Internal Availability Error:", err.message);
  ...
  try {
    ...
    if (bookingRes.ok) {
      ...
      return res.status(500).json({
        status: "error",
        message: "That time slot is no longer available. Could we try another time?",
        details: errorData // Helpful for debugging Vercel logs
      });
    }
